...
iF2_resid <- plot(fitted(i.F2.model), residuals(i.F2.model))
#homoskedastic

iF2_resid2 <- qqnorm(residuals(i.F2.model))
iF2_resid3 <- qqline(residuals(i.F2.model))
...
uF0_resid <- plot(fitted(u.F0.model), residuals(u.F0.model))
#bimodally distributed, centering the data does not help

uF0_resid2 <- qqnorm(residuals(u.F0.model))
uF0_resid3 <- qqline(residuals(u.F0.model))
...
#check residuals
uF0_resid_log <- plot(fitted(u.F0.model_log), residuals(u.F0.model_log))
#same; heteroskedastic
...
iF0_resid <- plot(fitted(i.F0.model), residuals(i.F0.model))
#bimodally distributed

iF0_resid2 <- qqnorm(residuals(i.F0.model))
iF0_resid3 <- qqline(residuals(i.F0.model))
...
##logistic regression; first center/rescale the values
ucent <- udata %>%
...
ul1_sum <- summary(u.model.logit1)
#main effect of f0; marginally significant effect of duration
...
#main effects of f0 and duration; marginally significant effect of F2

il2_sum <- summary(i.model.logit2)
#main effects of f0 and duration; marginally significant effect of F2

il3_sum <- summary(i.model.logit3)
#main effects of f0 and duration; marginally significant effect of F2
